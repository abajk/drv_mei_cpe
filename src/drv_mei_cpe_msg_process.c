...
MEI_STATIC IFX_void_t MEI_Recv_MODEM_EVT_TC(
                              ...
                              CMV_STD_MESSAGE_T *pMsg)
{
   MEI_InternalTcLayerRequestCallback_t MEI_InternalTcLayerRequestCallback;
      ...
      )
   {
      ...
      MEI_InternalTcLayerRequestCallback =
         pMeiDev->meiCallback.pTcLayerRequestHandler;
      if (MEI_InternalTcLayerRequestCallback)
      {
         MEI_InternalTcLayerRequestCallback(
            (IFX_int32_t)(MEI_DRV_LINENUM_GET(pMeiDev)), nApiTcUsed);
      }
      else
      {
         PRN_ERR_INT_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_HIGH,
              ("MEI_DRV[%02d]: EVT_TC_StatusGet_t - callback is not set"
